...
Usage:
    ...
    from simple_ui_client.features.doc_converter.worker import DocConverterWorker
    worker = DocConverterWorker(event_bus)
    await worker.start()
    
    # For CLI batch processing
    from simple_ui_client.features.doc_converter.worker import BatchWorker
...
from simple_ui_client.features.doc_converter.worker.processor import DocumentProcessor
